...
namespace LunaVK.UC
{
    public class ImageViewer : Canvas
    {
        ...
        public static readonly EasingFunctionBase ANIMATION_EASING;
        public static readonly EasingFunctionBase ANIMATION_EASING_IN_OUT;
        ...
        private Windows.UI.Xaml.Shapes.Rectangle _blackRectangle;
...
//        private bool _initialStatusBarVisibility;
        ...
        private Windows.UI.Xaml.Shapes.Rectangle _manipulationRectangle;
        ...
        */
        private PageBase Page
        {
            get
            {
                if (this._page == null)
                {
                    ...
                    while (!(frameworkElement is PageBase))
                        frameworkElement = frameworkElement.Parent as FrameworkElement;
                    this._page = frameworkElement as PageBase;
                }
                return this._page;
            }
        }
        ...
        static ImageViewer()
        {
            ...
            ImageViewer.ANIMATION_EASING = cubicEase;
            QuadraticEase quadraticEase = new QuadraticEase();
            quadraticEase.EasingMode = EasingMode.EaseInOut;
            ImageViewer.ANIMATION_EASING_IN_OUT = quadraticEase;
            ImageViewer.MARGIN_BETWEEN_IMAGES = 12.0;
            ImageViewer.MOVE_TO_NEXT_VELOCITY_THRESHOLD = 100.0;
            ImageViewer.DURATION_BOUNCING = 175;
            ImageViewer.DURATION_MOVE_TO_NEXT = 200;
            //ImageViewer.EPS = 0.0001;
            ImageViewer.MIN_SCALE = 0.5;
            ImageViewer.MAX_SCALE = 4.0;
            ImageViewer.HIDE_AFTER_VERT_SWIPE_THRESHOLD = 100.0;
            ImageViewer.HIDE_AFTER_VERT_SWIPE_VELOCITY_THRESHOLD = 100.0;
        }
        ...
        public ImageViewer()
        {
            ...
            this.Tapped += this.ImageViewer_Tap;
            this.Opacity = 0.0;
            this.Loaded += new RoutedEventHandler(this.ImageViewer_Loaded);
            this._imageAnimator = new ImageAnimator(ImageViewer.ANIMATION_INOUT_DURATION_MS, ImageViewer.ANIMATION_EASING_IN_OUT);
        }
        ...
        private static string GetImageSource(Image image)
        {
            ...
            //
            return (image.Source as BitmapImage).UriSource.AbsoluteUri;
...
//                ImageViewerLowProfileImageLoader.SetUriSource(image, null);
            ...
            else
            {
                ...
                if (!(ImageViewer.GetImageSource(image).ToString() != uri.ToString()))
                    return;
...
//                return;
        ...
        private void ImageViewer_Tap(object sender, TappedRoutedEventArgs e)
        {
...
//            }
        ...
        {
            if (increment)
                ++ImageViewer._shownImageViewersCount;
            else if (ImageViewer._shownImageViewersCount > 0)
                --ImageViewer._shownImageViewersCount;
//            if (ImageViewer._shownImageViewersCount == 0)
...
//            SystemTray.IsVisible = false;
            ImageViewer.UpdateShownCount(true);
            this.EnsurePrepareImages();
            this.ArrangeImages();
            this.Opacity = 1.0;
            if (!galleryPhotosMode)
            {
                ImageViewer.SetImageSource(this.CurrentImage, null);
            ...
            else
            {
                if (currentImage == null)
                    ImageViewer.SetImageSource(this.CurrentImage, this.GetImageSource(this.CurrentInd, false));
                else
                    ImageViewer.SetImageSource(this.CurrentImage, (object)currentImage);
                this._showHideOriginalImageAction(this.CurrentInd, false);
            }
            ...
            this._blackRectangle.Animate(this._blackRectangle.Opacity, 1.0, "Opacity", ImageViewer.ANIMATION_INOUT_DURATION_MS, 0, ImageViewer.ANIMATION_EASING, null);
...
//            DeviceOrientationHelper.Instance.OrientationChanged -= new EventHandler<DeviceOrientationChangedEventArgs>(this.Instance_OrientationChanged);
//            ImageViewerLowProfileImageLoader.ImageDownloaded -= new EventHandler(this.ImageViewerLowProfileImageLoader_ImageDownloaded);
            ImageViewer.UpdateShownCount(false);
...
//                clockwiseRotation = new bool?(false);
            ...
            if (!leavingPageImmediately)
            {
                ...
                this._blackRectangle.Animate(this._blackRectangle.Opacity, 0.0, "Opacity", ImageViewer.ANIMATION_INOUT_DURATION_MS, 0, null, null);
            }
            else
            {
                if (callback == null)
        ...
        private void i_ManipulationDelta(object sender, ManipulationDeltaRoutedEventArgs e)
        {
            e.Handled = true;
 ...
 //               this.HandlePinch(e.PinchManipulation.Current.PrimaryContact, e.PinchManipulation.Current.SecondaryContact);
        ...
        private void i_ManipulationCompleted(object sender, ManipulationCompletedRoutedEventArgs e)
        {
        ...
        private void HandleDragCompleted(double hVelocity, double vVelocity)
        {
            ...
            if (this._mode == ImageViewerMode.Normal)
            {
                if (this.CurrentImageScale == 1.0)
                {
                    ...
                    {
                        CompositeTransform currentImageTransform = this.CurrentImageTransform;
                        if (Math.Abs(currentImageTransform.TranslateY) < ImageViewer.HIDE_AFTER_VERT_SWIPE_THRESHOLD && num2 < ImageViewer.HIDE_AFTER_VERT_SWIPE_VELOCITY_THRESHOLD)
                        {
                            currentImageTransform.Animate(currentImageTransform.TranslateY, 0.0, "TranslateY", ImageViewer.ANIMATION_DURATION_MS, 0, ImageViewer.ANIMATION_EASING, null);
                            this._blackRectangle.Animate(this._blackRectangle.Opacity, 1.0, "Opacity", ImageViewer.ANIMATION_DURATION_MS, 0, null, null);
                            this._loadingTextBlock.Opacity = 1.0;
                        }
                        else
                            this.PerformHide();
                    ...
                    else
                    {
                        ...
                        double num3 = num1;
                        if ((num3 < -ImageViewer.MOVE_TO_NEXT_VELOCITY_THRESHOLD && translateX < 0.0 || translateX <= -this.Width / 2.0) && this._currentInd < this._count - 1)
                            moveNext = new bool?(true);
                        else if ((num3 > ImageViewer.MOVE_TO_NEXT_VELOCITY_THRESHOLD && translateX > 0.0 || translateX >= this.Width / 2.0) && this._currentInd > 0)
                        ...
                        {
                            num4 = -this.Width - ImageViewer.MARGIN_BETWEEN_IMAGES;
                        }
                        else
                        {
        ...
        private void AnimateTwoImagesOnDragComplete(Image image1, Image image2, double delta, Action completedCallback, bool movingToNextOrPrevious)
        {
            ...
            int num = movingToNextOrPrevious ? ImageViewer.DURATION_MOVE_TO_NEXT : ImageViewer.DURATION_BOUNCING;
            ...
            animInfoList.Add(new AnimationUtils.AnimationInfo()
            {
                ...
                easing = ImageViewer.ANIMATION_EASING
            });
            animInfoList.Add(new AnimationUtils.AnimationInfo()
            {
        ...
        {
            int duration = movingToNextOrPrevious ? ImageViewer.DURATION_MOVE_TO_NEXT : ImageViewer.DURATION_BOUNCING;
            CompositeTransform target = image.RenderTransform as CompositeTransform;
            target.Animate(target.TranslateX, target.TranslateX + delta, "TranslateX", duration, 0, ImageViewer.ANIMATION_EASING, completedCallback);
        }
        /*
        ...
        */
        private void HandlePinch(Point pointFirst, Point pointSecond)
        {
            ...
            double x4 = point2.X;
            double y4 = point2.Y;
            double x5 = ImageViewer.Clamp(Math.Sqrt(((x4 - x3) * (x4 - x3) + (y4 - y3) * (y4 - y3)) / ((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1))) * this._previousTransformation.ScaleX, ImageViewer.MIN_SCALE / 2.0, this.MaxScale);
            if (x5 < 1.0)
                x5 = ImageViewer.TranslateInterval(x5, ImageViewer.MIN_SCALE / 2.0, 1.0, ImageViewer.MIN_SCALE, 1.0);
        ...
        public void AnimateImage(double toScaleX = 1.0, double toScaleY = 1.0, double toTranslateX = 0.0, double toTranslateY = 0.0, Action completionCallback = null)
        {
            ...
            target.Animate(target.ScaleX, toScaleX, "ScaleX", ImageViewer.DURATION_BOUNCING, 0, ImageViewer.ANIMATION_EASING, null);
            target.Animate(target.ScaleY, toScaleY, "ScaleY", ImageViewer.DURATION_BOUNCING, 0, ImageViewer.ANIMATION_EASING, null);
            target.Animate(target.TranslateX, toTranslateX, "TranslateX", ImageViewer.DURATION_BOUNCING, 0, ImageViewer.ANIMATION_EASING, null);
            target.Animate(target.TranslateY, toTranslateY, "TranslateY", ImageViewer.DURATION_BOUNCING, 0, ImageViewer.ANIMATION_EASING, completionCallback);
        }
        ...
        private void MoveToNextOrPrevious(bool next)
        {
            if (next)
            {
                ImageViewer.Swap(this._images, 0, 1);
                ImageViewer.Swap(this._images, 1, 2);
            }
            else
            {
                ImageViewer.Swap(this._images, 1, 2);
        ...
        {
            this._images[0].Clip = null;
            this._images[1].Clip = null;
            this._images[2].Clip = null;
            ...
            double num1 = -(this.Width + ImageViewer.MARGIN_BETWEEN_IMAGES);
            ...
            double num5 = this.Width + ImageViewer.MARGIN_BETWEEN_IMAGES;
        ...
        private void UpdateImagesSources(bool keepCurrentAsIs = false, bool? movedForvard = null)
        {
            ...
            {
                if (!keepCurrentAsIs)
                    ImageViewer.SetImageSource(this._images[1], this.GetImageSource(this._currentInd, false));
                ImageViewer.SetImageSource(this._images[0], null);
                ImageViewer.SetImageSource(this._images[2], null);
            }
            else
            {
                ...
                if ((!movedForvard.HasValue ? 1 : (!movedForvard.HasValue ? 0 : (!movedForvard.Value ? 1 : 0))) != 0)
                {
                    ...
                    ImageViewer.SetImageSource(this._images[0], null);
                    ImageViewer.SetImageSource(this._images[0], imageSource);
                }
                if (num == 0)
                    return;
                object imageSource1 = this.GetImageSource(this._currentInd + 1, true);
                ImageViewer.SetImageSource(this._images[2], null);
                ImageViewer.SetImageSource(this._images[2], imageSource1);
            }
        }
        ...
        {
            ImageViewer.SetImageSource(this._images[0], null);
            ImageViewer.SetImageSource(this._images[1], null);
            ImageViewer.SetImageSource(this._images[2], null);
        }
        ...
        private void EnsurePrepareImages()
        {
            if (!this._initializedImages)
            {
                ...
                var rectangle2 = new Windows.UI.Xaml.Shapes.Rectangle();
...
//                this._manipulationRectangle.DoubleTap += this._manipulationRectangle_DoubleTap;
                this._manipulationRectangle.ManipulationDelta += this.i_ManipulationDelta;
                this._manipulationRectangle.ManipulationCompleted += this.i_ManipulationCompleted;
                this.Children.Add(this._manipulationRectangle);
        ...
        private void _manipulationRectangle_DoubleTap(object sender, DoubleTappedRoutedEventArgs e)
        {
